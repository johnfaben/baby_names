...
df = pd.DataFrame(columns = ['year','Name','boys','girls'])
for file in os.listdir('C:/Users/jdfab/Dropbox/baby_names/Data'):
    if file[-4:] == '.csv':
        year = int(file[-8:-4])
        boys = pd.read_csv(file,encoding = 'cp1252',skiprows=6,usecols = [1,2])
        girls = pd.read_csv(file,encoding = 'cp1252',skiprows =6, usecols = [5,6])
...
print(data.sort_values('change')[['boys15-19','girls15-19','boys74-79','girls74-79','change']].head(10).to_csv())
print(data.sort_values('change',ascending =False)[['boys15-19','girls15-19','boys74-79','girls74-79','change']].head(10).to_csv())
